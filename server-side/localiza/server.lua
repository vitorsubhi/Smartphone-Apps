		...
		if vehicle[1] then
			if os.time() >= parseInt(vehicle[1]["tax"] + 24 * 7 * 60 * 60) then
				...
				if vRP.paymentFull(user_id,vehiclePrice) then
					...
					TriggerClientEvent("smartphone-app:Update",source,"requestPossuidos")
				else
...
AddEventHandler('onResourceStart', function(resource)
	if resource == "smartphone-app" then
		Wait(10000)
	    TriggerClientEvent("smartphone-app:updateVehicles",-1,typeCars,typeBikes,typeWorks,typeRental)
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYERLEAVE
